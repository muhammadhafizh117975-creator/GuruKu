...
export function getNeonDatabaseUrl(): string | null {
  ...
  return (
    ...
    env.DATABASE_URL ||
    localStorage.getItem('guruku_neon_db_url') ||
    null
  );
}
...
export function getNeonSql(): any {
  ...
  if (connectionString) {
    try {
      ...
      console.warn('Gagal menginisialisasi client Neon Serverless Postgres:', e);
    }
  }
  return null;
}

export function resetNeonClient(url: string) {
  if (url) {
    localStorage.setItem('guruku_neon_db_url', url);
    try {
      neonSqlInstance = neon(url);
    } catch (e) {
      console.warn('Gagal menginisialisasi Neon client baru:', e);
    }
  } else {
    localStorage.removeItem('guruku_neon_db_url');
    neonSqlInstance = null;
  }
}

// Full Neon Serverless Postgres SQL Migration Script Generator
export function generateNeonSQLScript(): string {
  return `-- ============================================================
-- MASTER SQL SCHEMA UNTUK APLIKASI GURUKU (NEON SERVERLESS POSTGRES)
-- Silakan jalankan script ini di Neon Console SQL Editor
-- ============================================================
...
-- 2. TABEL PROFILES (AKUN GURU & ADMIN)
...
-- 7. TABEL ATTENDANCE (PRESENSI / ABSENSI)
...
-- 8. TABEL TEACHING_JOURNALS (JURNAL MENGAJAR)
...
-- 9. TABEL TEACHING_MODULES (ARSIP MODUL AJAR / RPP)
...
-- 10. TABEL SYSTEM_SETTINGS (PENGATURAN KOP SURAT & MARGIN)
...
-- 11. INDEXES UNTUK PERFORMA OPTIMAL NEON POSTGRES
